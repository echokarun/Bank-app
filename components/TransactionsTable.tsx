      ...
      <Table>
         <TableHeader className="bg-#f9fafb ">
            <TableRow>
               ...
               <TableHead className="px- max-md:hidden">Channel</TableHead>
               <TableHead className="px- max-md:hidden">Category</TableHead>
         ...
         <TableBody>
            {transactions.map((t: Transaction) => {
               ...
               const isCebit = t.type === "cebit"

               return (
                  <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !over:bg-none !b`}>
                     ...
                     <TableCell className={`pl-2 pr-10 font-semibold ${isDebit || amount[0] === '-' ? "text-[#f04438]" : "text-[#039855]"} `}>
                        {isDebit ? `-${amount}` : isCebit ? amount : amount}
                     </TableCell>
